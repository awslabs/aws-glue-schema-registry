...
public class GlueSchemaRegistryAvroSchemaCoderTest {
    ...
    private static final byte[] actualBytes = new byte[]{12, 99, 8, 116, 101, 115, 116, 0, 20, 0, 12, 118, 105, 111, 108, 101, 116};
    ...
    public void testReadSchema_withValidParams_succeeds() throws IOException {
        when(mockInputStreamDeserializer.getSchemaAndDeserializedStream(any(InputStream.class))).thenReturn(userSchema);
        GlueSchemaRegistryAvroSchemaCoder glueSchemaRegistryAvroSchemaCoder = new GlueSchemaRegistryAvroSchemaCoder(mockInputStreamDeserializer);
        Schema resultSchema = glueSchemaRegistryAvroSchemaCoder.readSchema(buildByteArrayInputStream());
    ...
    public void testWriteSchema_withValidParams_succeeds(AWSSchemaRegistryConstants.COMPRESSION compressionType) throws IOException {
        ...
        GlueSchemaRegistryOutputStreamSerializer glueSchemaRegistryOutputStreamSerializer =
                new GlueSchemaRegistryOutputStreamSerializer(testTopic, configs, spySerializationFacade);
        GlueSchemaRegistryOutputStreamSerializer spyOutputStreamSerializer = spy(glueSchemaRegistryOutputStreamSerializer);
        doCallRealMethod().when(spyOutputStreamSerializer).registerSchemaAndSerializeStream(any(), any(), any());
     ...
     */
    @Test
    public void testWriteSchema_withoutAutoRegistration_throwsException() throws NoSuchFieldException, IllegalAccessException {
        configs.put(AWSSchemaRegistryConstants.SCHEMA_AUTO_REGISTRATION_SETTING, false);

        EntityNotFoundException entityNotFoundException = EntityNotFoundException
                .builder()
        ...
        assertThat(exception.getMessage(), equalTo(AWSSchemaRegistryConstants.AUTO_REGISTRATION_IS_DISABLED_MSG));
    }

    private void testForSerializedData(byte[] serializedData, UUID testGenericSchemaVersionId, AWSSchemaRegistryConstants.COMPRESSION compressionType) {
    ...
    private void configureAWSSchemaRegistryClientWithSerdeConfig(
            AWSSchemaRegistryClient awsSchemaRegistryClient, GlueSchemaRegistryConfiguration glueSchemaRegistryConfiguration)
