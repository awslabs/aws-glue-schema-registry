...
public class TimeLogicalTypeConverter implements TypeConverter {
    @Override
    ...
    public Object toConnect(final Schema schema,
                            ...
                            final JsonSchemaDataConfig jsonSchemaDataConfig) {
        if (!(value.isInt())) {
            throw new DataException("Invalid type for Time, underlying representation should be integer but was "
                                    + value.getNodeType());
        }
    ...
    public SchemaBuilder toConnectSchema(org.everit.json.schema.Schema jsonSchema,
        ...
        throw new DataException("Invalid type for Time, underlying type should be integer");
    }
}
